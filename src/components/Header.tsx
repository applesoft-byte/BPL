...
export const Header: React.FC<HeaderProps> = ({
  ...
  const organizerName = draft?.organizerName || 'Arif Iquebal';
  const organizerRole = draft?.organizerRole || 'Organizer';
        ...
        <div className="flex items-center gap-2.5 sm:gap-4">
          ...
          <div className="relative">
            <button
              ...
              className="flex items-center gap-2 pl-1 pr-2.5 py-1 rounded-full hover:bg-slate-100 border border-transparent hover:border-slate-200 transition-all focus:outline-none"
            >
              ...
              <div className="hidden sm:flex flex-col text-left leading-tight">
                <span className="text-xs font-bold text-[#061A36] truncate max-w-[110px]">
                  {organizerName}
                </span>
                <span className="text-[10px] text-slate-500 truncate">{organizerRole}</span>
              </div>
            ...
            {userDropdownOpen && (
              <div className="absolute right-0 mt-2 w-56 bg-white rounded-2xl shadow-xl border border-slate-200 py-1.5 z-50 text-xs animate-in fade-in zoom-in-95 duration-100">
                <div className="px-3.5 py-2 border-b border-slate-100">
                  <div className="font-bold text-[#061A36]">{organizerName}</div>
                  <div className="text-[11px] text-slate-500">{organizerRole}</div>
                </div>
